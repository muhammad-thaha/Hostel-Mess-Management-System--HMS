  ...
  return (
    <div className="space-y-10 max-w-6xl mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Mess Members List - hidden for students */}
        {user.role !== "STUDENT" && (
          <div className="lg:col-span-12 bg-white p-8 rounded-[2.5rem] border border-slate-50 shadow-xl shadow-slate-200/40 mb-8">
            <h3 className="font-black text-slate-900 text-lg mb-4 flex items-center">
              <Utensils size={20} className="mr-2 text-emerald-600" />
              Mess Members
            </h3>
                ...
                <tbody>
                  {members
                    .filter((m) => {
                      ...
                      if (user.role === "WARDEN") {
                        return m.role === "STUDENT" || m.role === "STAFF";
                      }
                      if (
                        user.role === "ADMIN" ||
                    ...
                    .map((m) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">{m.registerNumber || "-"}</td>
                        <td className="px-3 py-2">{m.department || "-"}</td>
                        <td className="px-3 py-2">{m.year || "-"}</td>
                        <td className="px-3 py-2">{m.messStatus || "-"}</td>
                      </tr>
                    ))}
                  {members.length === 0 && (
                    <tr>
